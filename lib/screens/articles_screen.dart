...
class ArticlesScreen extends StatefulWidget {
  ...
  State<StatefulWidget> createState() =>
      _ArticlesScreenState(/*articles: this.articles*/);
}
...
class _ArticlesScreenState extends State<ArticlesScreen> {
  ...
  void deleteArticle(int articleId) {
    ...
    container.articleData.articles
        .removeWhere((article) => article.id == articleId);
    container.articleData.bookmarkedArticles
        .removeWhere((article) => article.id == articleId);
    container.articleData.filteredArticles
        .removeWhere((article) => article.id == articleId);
  ...
  Future<void> _refreshData() async {
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        ...
        container.articleData.bookmarkedArticles =
            articles.where((element) => element.bookmarked == true).toList();
  ...
  Widget build(BuildContext context) {
    // Fixme move data fetch functional outside from the build method,
  ...
  Widget _buildWidgetListDataAndroid() {
    return RefreshIndicator(
      child: (displayedArticles.length > 0)
          ...
          : ListView(children: [
        // Fixme  alignment: Alignment.center could be used with container
        //  instead of the Center widget
        // Container(
        //     height: 360,
